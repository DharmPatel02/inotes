...
const Signup = (props) => {
  ...
  useEffect(() => {
    ...
    const handleCallbackRespnse = async (res) => {
      ...
      const response = await fetch(`${host}/api/auth/googlesignin`, {
        ...
        body: JSON.stringify({
          name: decoded_res.name,
          email: decoded_res.email,
          password: gpassword,
        }), // body datLink type must match "Content-Type" header
      });
    ...
    google.accounts.id.renderButton(document.getElementById("g_id_signup"), {
      theme: "filled_blue",
      size: "large",
      text: "signup_with",
    });
  ...
  const handleSignin = async (e) => {
    ...
    if (
      signcred.signpassword === signcred.confirmpass &&
      signcred.signpassword.length > 4 &&
      signcred.confirmpass.length > 4
    ) {
      const response = await fetch(`${host}/api/auth/createuser`, {
        ...
        body: JSON.stringify({
          name: fullname,
          email: signcred.signemail,
          password: signcred.confirmpass,
        }), // body datLink type must match "Content-Type" header
      });
      const json = await response.json();
      ...
      signcred.signpassword.length < 5 ||
      signcred.confirmpass.length < 5
    ) {
      setAlert({
        msg: "Password should be atleast 5 character",
        visible: "visible",
      });
      setTimeout(() => {
        setAlert({ msg: "", visible: "hidden" });
              ...
              <form>
                ...
                <div className="form-outline mb-4">
                  <input
                    ...
                    onChange={onChange}
                    value={signcred.signemail}
                  />
                </div>

                <div
                  className="form-outline mb-4 d-flex pass_show"
                ...
                >
                  <input
                    ...
                    onChange={onChange}
                    value={signcred.signpassword}
                  />
                  <FontAwesomeIcon
                    className="my-2"
                    icon={spasswordicon ? faEyeSlash : faEye}
                    style={{
                      position: "absolute",
                      marginRight: "15px",
                      paddingTop: "7px",
                    }}
                    onClick={handleSigninPass}
                  />
                </div>

                <div
                  className="form-outline mb-2 d-flex pass_show"
                  style={{ justifyContent: "end" }}
                >
                  <input
                    ...
                    onChange={onChange}
                    value={signcred.confirmpass}
                  />
                  <FontAwesomeIcon
                    className="my-2"
                    icon={cspasswordicon ? faEyeSlash : faEye}
                    style={{
                      position: "absolute",
                      marginRight: "15px",
                      paddingTop: "7px",
                    }}
                    onClick={handleCSigninPass}
                  />
                </div>
                <p
                  className="mx-2 sign-alert"
                  style={{
                    color: "red",
                    fontWeight: "bold",
                ...
                <button
                  type="submit"
                  className="btn btn-primary d-flex text-uppercase shadow p-3 mb-2 rounded"
                  style={{
                ...
                <div className="signup-social flex-row align-items-center justify-content-center justify-content-lg-start">
                  ...
                  <div
                    id="g_id_signup"
                    className="g_id_signup col social-google"
